...
// to run Matlab in Windows, it's path needs to be in the environment path
let add_matlab_to_path = require('./add_matlab_to_path');
if (add_matlab_to_path())
{
  electron.showErrorBox('MATLAB Not Found', 'MATLAB is not found. Cannot run the app.');
  return;
}

const addon = require("bindings")("./Release/addon.node");
ipcMain.on('get-pi', (event) => {
  let pi = addon.getPi();
  event.returnValue = pi.toString();
